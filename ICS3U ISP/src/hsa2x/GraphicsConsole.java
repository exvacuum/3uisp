 ...
 * 
 * Custom extended edition created by Silas Bartha in 2019, for personal use during the 
 * development of Thursday.
 *  
 * <p>
 ...
 * @author Silas Bartha (isMouseMoved, drawCenteredString, setBackgroundHybrid)
 * @version 1
 */
public class GraphicsConsole extends JFrame implements MouseListener, MouseMotionListener, MouseWheelListener, ComponentListener {
	...
	private boolean mouseDrag = false, mouseMove = false;
...
*/
	...
	/**
	 * Same as setBackgroundColor(), includes setBackground() as well.
	 * @param c New Background Color
	 */
	public void setBackgroundHybrid(Color c){
	...
	/**
	 * Draws a string on the drawing area using the current font and color.
	 * Note that the coordinates specify the bottom of the center of text. 
	 * AntiAliasing can be set using setAntiAlias()
	 * @param str The string to draw. 
	 * @param x Center X coordinate
	 * @param y Bottom Y coordinate
	 * @param f Font to be used to acquire metrics.
	 */
	public void drawCenteredString(String str, int x, int y, Font f) {
		FontMetrics fm = canvas.getFontMetrics(f);
		int cx = (x-fm.stringWidth(str)/2);
		canvas.drawString(str, cx, y);
	}
	/**
...
<pre>
	...
	*/
	...
	public void mousePressed(MouseEvent e) {
		mouseX = e.getX();
		mouseY = e.getY();
		mouseClick = e.getClickCount(); //moved to mouseReleased for faster response, SB moved back to resolve issues with drag.
		mouseMove = false;
	...
	public void mouseReleased(MouseEvent e) {
		mouseX = e.getX();
		mouseY = e.getY();
		//mouseClick = e.getClickCount(); //SB: Removed Due to Issues with dragging and releasing clicks.
